...
import (
	...
	pb "{{ .Package }}/api/{{ .ServiceLower }}/service/v1"
	"{{ .Package }}/app/router/{{ .Mode }}/internal/biz"
...
func New{{ .Service }}UseCase(repo {{ .Service }}Repo, client pb.{{ .Service }}Client, logger log.Logger) *{{ .Service }}UseCase {
	uc := &{{ .Service }}UseCase{repo: repo, client: client, log: log.NewHelper(logger)}
...
import (
	...
	"{{ .Package }}/api/{{ .ServiceLower }}/service/v1"
	"{{ .Package }}/app/router/{{ .Mode }}/internal/biz"
	"gitlab.wwgame.com/wwgame/kratos/v2/log"
