...
var CmdServer = &cobra.Command{
	Use:   "router",
	Short: "Generate the proto router implementations",
	Long:  "Generate the proto router implementations. Example: kratos proto router api/xxx.proto --target-dir=internal",
	Run:   run,
}
var (
	targetDir string
...
func run(_ *cobra.Command, args []string) {
	...
	if _, err := os.Stat(dataDir); os.IsNotExist(err) {
		fmt.Printf("data directory: %s does not exsit\n", dataDir)
		return
	}
	for _, s := range res {
...
func parametersName(name string) string {
	...
	return toUpperCamelCase(split[len(split)-1])
}
